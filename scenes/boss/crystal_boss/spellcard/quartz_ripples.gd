	...
	if (lifetime-1) % 60*2 == 0:
		...
		for bullet in bulletList:
			...
			if b.type == Level.BulletType.BALL:
				...
				_level.bullet_ring(pos, Level.BulletType.SMALL_BALL, 4, randf_range(0,PI/16), 1, 60*3)
			elif b.type == Level.BulletType.SMALL_BALL:
				var pos = b.global_position
				_level.clear_bullet(b, false)
				_level.bullet_burst(pos, Level.BulletType.SHARD, 3, deg_to_rad(90), (playerPos - pos).angle(), 0, 90*3, 0)
			
		_level.bullet_ring(global_position, Level.BulletType.BALL, 8, 0, 9, 80*3, 0)
